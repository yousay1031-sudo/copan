...
function submitToGoogleForm(name, phone, email, subject, message) {
    ...
    formData.append('entry.613536919', subject);        // お問い合わせ種類（修正）
    formData.append('entry.835707395', message);        // お問い合わせ内容（修正）
...
function loadPublicNews() {
    const newsData = localStorage.getItem('copan_public_news');
    
    if (!newsData) {
        // データがない場合はデフォルトのお知らせを表示（既存のHTMLがそのまま表示される）
        return;
    }
    ...
    if (newsList.length === 0) {
        // データがない場合はデフォルトのお知らせを表示
        return;
    }
    
    // お知らせを動的に生成（クリック可能なリンク付き）
    ...
    const viewAllLink = `
        <div style="text-align: center; margin-top: 30px;">
            <a href="news.html" style="display: inline-block; padding: 12px 30px; background: linear-gradient(135deg, #FF9F5A 0%, #FFB47A 100%); color: white; text-decoration: none; border-radius: 25px; font-weight: 600; transition: all 0.3s ease;">
                <i class="fas fa-newspaper"></i> すべてのお知らせを見る
            </a>
        </div>
    `;
    
    container.insertAdjacentHTML('afterend', viewAllLink);
}